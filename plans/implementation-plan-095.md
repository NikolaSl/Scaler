...
## Scope
- IMPL-359: Add runtime Pi tool introspection helpers that format only compact catalogs (name, short purpose, risk, docs/schema availability, active flag) without parameter schemas or prompt guidelines.
- IMPL-360: Add parent-session active-tool focus/restore helpers and command/UI affordances that snapshot active tools, narrow requester turns to SCALER requester tools, and restore the original active set.
- IMPL-361: Wire automatic focus/catalog injection into the Pi `context`/turn lifecycle for SCALER-guided turns, with audit logging and tests proving full schemas/docs are not injected.
