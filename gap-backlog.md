...
| GAP-012 | PRD-S08, PRD-S09, PRD-P04 | Tool/MCP isolation now has compact default/selected tool catalogs, richer structured `scaler_tool_request` metadata, selected-tool prompt isolation, mocked integration coverage, and opt-in real Pi cardinal tool-request coverage. Remaining gaps: automatic MCP docs/schema discovery, focused multi-iteration tool-agent execution, result-report ingestion, and transaction records. | Add tool-agent result schema/ledger and focused tool transaction execution records, then MCP docs/schema discovery. |
