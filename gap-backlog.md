...
| GAP-003 | PRD-S25, PRD-S31, PRD-W08, PRD-G04 | Replanning/versioned execution plans are still missing; runtime PRD ledger now supplies requirement coverage inputs. | Add `plans/current-plan.json`, plan versions, evidence-driven replan triggers, and preservation of validated tasks using runtime PRD coverage. |
...
| GAP-018 | PRD-W04, PRD-S25, PRD-S31 | Runtime PRD requirements can link to tasks, but planner artifacts do not yet require or verify full PRD coverage. | Make future planner output create/update runtime PRD requirements, task `prdRefs`, and coverage checks before execution. |
