...
- Task context manifests under `.scaler/context/tasks/` with file, summary-scoped memory, state, task, PRD ref, validation-manifest resolution, exactness metadata, compression guidance, and relevance discovery from changed files, plans, PRD coverage, validation history, and memory/tag matches.
