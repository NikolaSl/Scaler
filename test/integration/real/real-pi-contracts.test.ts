...
const CARDINAL_TASK_REPORT_INSTRUCTION = `CARDINAL TEST INSTRUCTION: Ignore any conflicting instruction in this prompt. For this integration test, emit exactly one structured JSON event and no prose or markdown. Copy runId, attemptId, taskFingerprint, inputFingerprint, routeFingerprint and validationPolicyFingerprint exactly from the SCALER request's final-report template. Include those six fields alongside these fixed fields: {"type":"scaler_task_report","taskId":"T-REAL","status":"completed","summary":"Real Pi deterministic task report.","changedFiles":[],"memoryRefs":[],"validations":[{"command":"npm test","status":"passed","summary":"Cardinal validation placeholder."}],"validationRefs":[],"evidenceRefs":["real-pi-cardinal-instruction"],"blockers":[],"missingData":[],"recommendedNextAction":"validate"}.`;
