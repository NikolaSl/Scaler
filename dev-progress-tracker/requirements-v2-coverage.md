...
host-selected API/provider/model/context-window identity; explicit spawn ignores
model-authored selection authority. The shared launcher renders explicit
provider and model selectors, the final provider hook rechecks the same
identity, and attempt fingerprints use the qualified route. Isolated tool
execution keeps its fresher supplier binding. SC-05/25 remain Partial and SC-09
Not assessed because parent interactive calls, eligibility policy and a
supported real local-model outcome are not established.

PLAN-131 makes loaded-tool availability an intrinsic strict-child invariant.
The shared invocation rejects browser/MCP/custom grants that its isolated
Pi/SCALER loader cannot provide even when the legacy opt-in is omitted or false;
malformed and sparse runtime grants fail with structured admission errors.
Stage, conductor, debug-retry and isolated tool dispatch refuse before runner,
attempt, spawned-agent budget, prepared-run or execution-claim publication.
Build, 1,014 unit/component, 67 mock integration, 7 conformance/autopilot and
169 focused checks pass. SC-05 remains Partial because the trusted external
capability adapter and the other model-call boundaries listed above remain open.
